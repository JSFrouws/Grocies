const { areCompatible, convert, normalizeUnit } = require('./units');

class ShoppingListService {
    constructor(db, mappingService, stockService, recurringService) {
        this.db = db;
    ...
    generateShoppingList() {
        ...
        for (const [key, data] of ingredientMap) {
            // Try to find mapping first so we can aggregate in preferred unit
            const mapping = this.mappingService.getPreferredMapping(data.originalName);
            const preferredUnit = mapping ? mapping.package_unit : null;
            const aggregated = this.aggregateQuantities(data.quantities, preferredUnit);
            ...
            if (mapping) {
                ...
                const mappingUnit = normalizeUnit(mapping.package_unit || '');
                let inStock = 0;
                ...
                if (this.stockService) {
                    ...
                    if (stockEntries[stockKey]) {
                        for (const entry of stockEntries[stockKey]) {
                            ...
                            const neededUnit = normalizeUnit(totalUnit);
                            if (!neededUnit || !entryUnit || areCompatible(neededUnit, entryUnit)) {
                                let stock = entry.total;
                                if (neededUnit && entryUnit && neededUnit !== entryUnit) {
                                    const c = convert(stock, entryUnit, neededUnit);
                                    if (c !== null) stock = c;
                                }
                                inStock += stock;
                            }
                ...
                if (mapping.package_amount && mapping.package_amount > 0 && netNeeded > 0) {
                    // Convert netNeeded to mapping package unit if different
                    let netInPkgUnit = netNeeded;
                    if (mappingUnit && totalUnit && mappingUnit !== normalizeUnit(totalUnit)) {
                        const c = convert(netNeeded, totalUnit, mappingUnit);
                        if (c !== null) netInPkgUnit = c;
                    }
                    packagesNeeded = Math.ceil(netInPkgUnit / mapping.package_amount);
                }
        ...
        // Get recurring items
        const recurringItems = this.recurringService
            ? this.recurringService.getActiveItems().map(item => ({
                id: item.id,
                itemName: item.item_name,
                category: item.category,
                quantity: item.quantity,
                occurrenceRate: item.occurrence_rate,
                jumboSku: item.jumbo_sku,
                jumboProductId: item.jumbo_product_id,
    ...
    // Aggregate quantities, merging compatible units (g+kg, mL+L)
    aggregateQuantities(quantities, preferredUnit) {
        // If we have a preferred unit from mapping, convert everything to that unit
        const targetUnit = preferredUnit ? normalizeUnit(preferredUnit) : null;

        // Group by unit family, converting to target unit if possible
        const byFamily = new Map(); // family -> { unit, amounts[] }

        for (const q of quantities) {
            const norm = normalizeUnit(q.unit);

            if (targetUnit && areCompatible(norm, targetUnit)) {
                const converted = convert(q.amount, norm, targetUnit);
                if (converted !== null) {
                    if (!byFamily.has(targetUnit)) byFamily.set(targetUnit, []);
                    byFamily.get(targetUnit).push(converted);
                    continue;
                }
            }

            // Fallback: group by original unit
            if (!byFamily.has(norm || q.unit)) byFamily.set(norm || q.unit, []);
            byFamily.get(norm || q.unit).push(q.amount);
        }

        const aggregated = [];
        for (const [unit, amounts] of byFamily) {
            const total = amounts.reduce((sum, amount) => sum + amount, 0);
            aggregated.push({
                amount: this.roundQuantity(total, unit),
                unit: unit
            });
        }
    ...
    // Round quantity to sensible display values
    roundQuantity(amount, unit) {
        const norm = normalizeUnit(unit);
        // Auto-scale: 1500g -> 1.5kg, 2000mL -> 2L
        if (norm === 'g' && amount >= 1000) {
            return Math.round(amount / 100) / 10; // returns kg value but caller keeps unit
        }
        if (norm === 'mL' && amount >= 1000) {
    ...
    exportAsText(shoppingList) {
        ...
        if (shoppingList.recurringItems && shoppingList.recurringItems.length > 0) {
            text += '\uD83D\uDD01 Vaste boodschappen:\n';
            text += '-'.repeat(50) + '\n';
            shoppingList.recurringItems.forEach(item => {
                let line = `\u2022 ${item.itemName} (${item.quantity}x)`;
                if (item.productDetails?.title) {
                    line += ` \u2192 ${item.productDetails.title}`;
                }
                text += line + '\n';
            });
        }

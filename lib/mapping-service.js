class MappingService {
    ...
    // Get mapped ingredient names with their preferred defaults (unit, amount, shelf life)
    getMappedIngredientDefaults() {
        const stmt = this.db.prepare(`
            SELECT ingredient_name, package_amount, package_unit, shelf_life_days,
                   jumbo_sku, product_details
            FROM ingredient_mappings
            WHERE preferred = 1
            ORDER BY ingredient_name ASC
        `);
        return stmt.all().map(row => ({
            ...
            jumbo_sku: row.jumbo_sku,
            product_title: row.product_details ? (JSON.parse(row.product_details).title || null) : null
        }));
    }

...
class MealPlannerDatabase {
    ...
    initializeSchema() {
        ...
        // Recurring items migrations
        try {
            this.db.exec(`ALTER TABLE recurring_items ADD COLUMN mapping_id INTEGER REFERENCES ingredient_mappings(id)`);
        } catch (e) { /* already exists */ }
        try {
            this.db.exec(`ALTER TABLE recurring_items ADD COLUMN quantity INTEGER DEFAULT 1`);
        } catch (e) { /* already exists */ }
        try {
            this.db.exec(`ALTER TABLE recurring_items ADD COLUMN enabled BOOLEAN DEFAULT 1`);
        } catch (e) { /* already exists */ }
        // Legacy columns from earlier version (kept for migration safety)
        try { this.db.exec(`ALTER TABLE recurring_items ADD COLUMN jumbo_sku TEXT`); } catch (e) { /* */ }
        try { this.db.exec(`ALTER TABLE recurring_items ADD COLUMN jumbo_product_id TEXT`); } catch (e) { /* */ }
        try { this.db.exec(`ALTER TABLE recurring_items ADD COLUMN product_details TEXT`); } catch (e) { /* */ }

...
class StockService {
    ...
    // Converts between compatible units (g/kg, mL/L)
    consumeFromStock(ingredientName, amountNeeded, unit) {
        ...
        for (const item of stockItems) {
            ...
            // Skip incompatible units (weight vs volume vs count)
            if (stockUnit && normUnit && !areCompatible(stockUnit, normUnit)) continue;

            // Convert remaining needed to stock's unit for comparison
            let neededInStockUnit = remaining;
            if (stockUnit && normUnit && stockUnit !== normUnit) {
                neededInStockUnit = convert(remaining, normUnit, stockUnit);
                if (neededInStockUnit === null) continue;
            }

            const available = item.quantity_remaining;
            const consumed = Math.min(available, neededInStockUnit);
            ...
            // Convert consumed back to recipe unit for tracking
            let consumedInRecipeUnit = consumed;
            if (stockUnit && normUnit && stockUnit !== normUnit) {
                consumedInRecipeUnit = convert(consumed, stockUnit, normUnit);
            }

            results.push({
                stockId: item.id,
                ingredientName: item.ingredient_name,
                consumed: consumedInRecipeUnit,
            ...
            for (const ing of ingredients) {
                ...
                if (stockEntries) {
                    ...
                    for (const entry of stockEntries) {
                        ...
                        if (!ingUnit || !entryUnit || areCompatible(ingUnit, entryUnit)) {
                            // Convert needed to stock unit for comparison
                            let neededConverted = needed;
                            if (ingUnit && entryUnit && ingUnit !== entryUnit) {
                                const c = convert(needed, ingUnit, entryUnit);
                                if (c !== null) neededConverted = c;
                            }
                            if (entry.total >= neededConverted) {
                                hasStock = true;
                                break;
                            }
                        }

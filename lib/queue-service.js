class QueueService {
    ...
    addRandomRecipe(avoidDays = 14) {
        ...
        let query = 'SELECT id, name, frequency_weight FROM recipes';
        ...
        const selectedRecipe = this.weightedRandomSelection(availableRecipes);
    ...
    consumeRecipe(id, rating = null, notes = null) {
        const item = this.getQueueItemById(id);
        if (!item) {
            return null;
        }
